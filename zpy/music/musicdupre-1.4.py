#!/usr/bin/env python3
# Script Name: New-Music-Duplicate-remover-1.5.py
# ID: SCR-ID-20260317131020-CMTWG0ZLX3
# Assigned with:
# Created by: Tyler Jensen
# Email: tylerjensen5@yahoo.com
# Alias Call: New-Music-Duplicate-remover-1.5
...
- -d/--defaultdir --on [path]   enable + set default (path optional; uses cwd)
...
def run_dedupe(
    ...
    kept_map: dict[str, str] = {}  # sha256 -> kept path
    duplicates: list[str] = []
...
def main() -> int:
    ...
    # default dir toggle (settings command)
    parser.add_argument("-d", "--defaultdir", action="store_true",
                        help="Use/manage a saved default scan directory (stored next to script in logs/default_dir.csv).")
    parser.add_argument("--on", dest="defaultdir_on", action="store_true",
                        help="With -d: enable defaultdir mode (and optionally set the default path).")
    parser.add_argument("--off", dest="defaultdir_off", action="store_true",
    ...
    if args.defaultdir:
        ...
        new_default = provided if provided else str(Path.cwd())

        try:
            new_default = str(Path(new_default).expanduser().resolve())
        except Exception:
            new_default = os.path.expanduser(new_default)
    ...
    if use_active:
        ...
        # If nothing provided, try saved default dir if enabled
        if not raw:
    ...
    # Optional pc-like actions on scan dir
    if args.copy:
        if not conv2:
