...
# URL Extractor (Non-Destructive / Archive Mode)
...
# PROCESS (READ ONLY)
# -----------------------
def process_files(files, archive_dir: Path):
    urls = set()
    ...
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_file = archive_dir / f"exported-tabs_{ts}.txt"

    out_file = unique_file(out_file)

    out_file.write_text("\n".join(sorted_urls) + "\n", encoding="utf-8")

    print(f"\n✅ Exported {len(sorted_urls)} URLs")
    print(f"📁 Archive: {out_file}")
...
def main():
    ...
    print(f"\n✅ Active root: {root}")
    print(f"📂 Reading from: {brave_dir}")
    print(f"📄 Files detected: {len(files)}")

    archive_dir = root / "archives"

    process_files(files, archive_dir)
