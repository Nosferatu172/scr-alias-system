...
def create_archive(source, archive, dry, silent)
  ...
  if has_pv? && ENV_TYPE != :windows
    ...
    system("tar -cf - -C \"#{source}\" . | pv -s #{size} | gzip > \"#{archive}\"")
  else
...
def edit_entries(entries)
  ...
  loop do
    ...
    menu_size = [height - 5, choices.size].min
...
def show_help
  ...
  NOTES:
    - Uses tar + pv (if installed)
    - Works in WSL, Linux, Windows
    - Ctrl+C safe exit
