...
DEFAULT_INPUT = '/mnt/c/scr/keys/tabs/new/exported-tabs.txt'
DEFAULT_VIDEO = '/c/Users/tyler/Music/y-hold/'
...
threads = Array.new(max_threads) do
  Thread.new do
    until queue.empty?
      ...
      cmd = "yt-dlp --ffmpeg-location "C:\msys64\mingw64\bin\ffmpeg.exe" -S 'res,ext:mp4:m4a' --recode mp4 -o '#{out}/%(title)s.%(ext)s' #{url}"
      puts "🎯 #{url}"
      system(cmd)
    end
  end
