...
BASE_RB="/mnt/c/scr/zru"
BASE_ALIAS="/mnt/c/scr/aliases/lib/act"
TEMPLATE_FILE="$BASE_RB/.template.txt"
...
Examples:
  .newrb.sh weather
  .newrb.sh -c testtool
  .newrb.sh -r file-ops weather
  .newrb.sh -r file-ops/music/renamer weather
  .newrb.sh -a basic finder
  .newrb.sh -e weather
EOF
}
...
pick_main_mode() {
    local choice
    echo "Create Ruby script in:"
    echo "  1) Current directory"
    echo "  2) Ruby tree ($BASE_RB)"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -r)
            shift
            [ $# -eq 0 ] && echo "Missing subdir after -r" && exit 1
            TARGET_MODE="ruby"
            TARGET_SUBDIR="$1"
            shift
            ;;
...
case "$TARGET_MODE" in
    ...
    ruby)
        if [ -z "${TARGET_SUBDIR:-}" ]; then
            if pick_subdir_mode "Ruby"; then
                :
            fi
            mode_result=$?
            if [ "$mode_result" -eq 1 ]; then
                pick_top_level_subdir "$BASE_RB"
            elif [ "$mode_result" -eq 2 ]; then
                prompt_relative_subpath "$BASE_RB"
            fi
        fi
        TARGET="$BASE_RB/$TARGET_SUBDIR/$NAME"
        ;;
    alias)
        if [ -z "${TARGET_SUBDIR:-}" ]; then
            ...
            mode_result=$?
            if [ "$mode_result" -eq 1 ]; then
                pick_top_level_subdir "$BASE_ALIAS"
            elif [ "$mode_result" -eq 2 ]; then
                prompt_relative_subpath "$BASE_ALIAS"
            fi
        fi
