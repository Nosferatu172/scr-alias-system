#!/usr/bin/env bash
# Script Name: iosdcim2normal-multicore-1.1.sh
# ID: SCR-ID-20260329043018-2DO54B1YZQ
# Assigned with:
# Created by: Tyler Jensen
# Email: tylerjensen5@yahoo.com
# Alias Call: iosdcim2normal-multicore-1.1
...
# Find all MOV, HEIC, HEIF, WEBP files recursively
FILES=$(find "$SRC_DIR" -type f \( -iname "*.mov" -o -iname "*.heic" -o -iname "*.heif" -o -iname "*.webp" \))

if [[ -z "$FILES" ]]; then
    echo "No MOV, HEIC, HEIF, or WEBP files found in $SRC_DIR"
    exit 0
fi
